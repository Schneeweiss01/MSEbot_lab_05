...
//   NEO_RGBW    Pixels are wired for RGBW bitstream (NeoPixel RGBW products)
   ...
   const int servoChannel = 7;                  // select the channel used for the RC servo motor
   int servoPos=0;
   ...
   switch(CR1_ucMainTimerCaseCore1)  //full switch run through is 1mS
   {
    ...
    case 0: 
    {
      ...
      if (btRun)
      {
        ...
        {
           if( ENC_vi32RightOdometer <240)//start correction
            {
            //set up a boolean of should be forward should be backwards
            //if the encoders should be forward
            ...
            /*
            else if( ENC_vi32RightOdometer <225)                                                            //HAVE TO CODE THIS NUMBER
            {
            //set up a boolean of should be forward should be backwards
            //if the encoders should be forward
            ENC_SetDistance(100, 100);
            ...
            */
            else
            {
              ...
              if(CR1_ulMotorTimerNow-breakTimer >=500)
              {
              state = 1;
              }
            }
          ...
          {
            CR1_ulMotorTimerNow=millis();
            if(CR1_ulMotorTimerNow - CR1_ulMotorTimerPrevious <= 315)//after X seconds go to next part     //HAVE TO CODE THIS NUMBER
          {
          ...
          {
           if( ENC_vi32RightOdometer <210)//start correction
            {
            //set up a boolean of should be forward should be backwards
            //if the encoders should be forward
            ENC_SetDistance(100, 100);
            ...
            /*
            else if( ENC_vi32RightOdometer <240)                                                            //HAVE TO CODE THIS NUMBER
            {
            //set up a boolean of should be forward should be backwards
            //if the encoders should be forward
            ENC_SetDistance(100, 100);
          ...
          {
            CR1_ulMotorTimerNow=millis();
            if(CR1_ulMotorTimerNow - CR1_ulMotorTimerPrevious <= 540)//after X seconds go to next part     //HAVE TO CODE THIS NUMBER
          {
         ...
         else if(state ==4)//Move Forward
          {
            if( ENC_vi32RightOdometer <290)//start correction
            {
            //set up a boolean of should be forward should be backwards
            //if the encoders should be forward
            ENC_SetDistance(100, 100);
            ...
            /*
            else if( ENC_vi32RightOdometer <200)                                                            //HAVE TO CODE THIS NUMBER
            {
            //set up a boolean of should be forward should be backwards
            //if the encoders should be forward
            ENC_SetDistance(100, 100);
          ...
          else if(state ==5)//Turn Right
          {
            CR1_ulMotorTimerNow=millis();
            if(CR1_ulMotorTimerNow - CR1_ulMotorTimerPrevious <= 470)//after X seconds go to next part     //HAVE TO CODE THIS NUMBER
          {
            ENC_SetDistance(-(ci8RightTurn), ci8RightTurn);
            CR1_ui8LeftWheelSpeed = CR1_ui8WheelSpeed;
            CR1_ui8RightWheelSpeed = CR1_ui8WheelSpeed;
         ...
         else if(state ==6)//Move Forward
          {
            if( ENC_vi32RightOdometer <40)//start correction
            {
            //set up a boolean of should be forward should be backwards
            //if the encoders should be forward
            ENC_SetDistance(100, 100);
            ...
            else if( ENC_vi32RightOdometer <295)                                                            //HAVE TO CODE THIS NUMBER
            {
            //set up a boolean of should be forward should be backwards
            //if the encoders should be forward
            ENC_SetDistance(100, 100);
          ...
          else if(state ==7)//Turn Left
          {
            CR1_ulMotorTimerNow=millis();
            if(CR1_ulMotorTimerNow - CR1_ulMotorTimerPrevious <= 370)//after X seconds go to next part     //HAVE TO CODE THIS NUMBER
          {
            ENC_SetDistance(-(ci8LeftTurn), ci8LeftTurn);
            CR1_ui8LeftWheelSpeed = CR1_ui8WheelSpeed;
            CR1_ui8RightWheelSpeed = CR1_ui8WheelSpeed;
          ...
          else if(state == 8)//Turn Right
          {
            CR1_ulMotorTimerNow=millis();
            if(CR1_ulMotorTimerNow - CR1_ulMotorTimerPrevious <= 930)//after X seconds go to next part     //HAVE TO CODE THIS NUMBER
          {
            ENC_SetDistance(-(ci8RightTurn), ci8RightTurn);
            CR1_ui8LeftWheelSpeed = CR1_ui8WheelSpeed;
            CR1_ui8RightWheelSpeed = CR1_ui8WheelSpeed;
          ...
          else if(state ==9)//Move Forward
          {
           if( ENC_vi32RightOdometer <40)//start correction
            ...
            else if( ENC_vi32RightOdometer <100)                                                            //HAVE TO CODE THIS NUMBER
            {
            //set up a boolean of should be forward should be backwards
            //if the encoders should be forward
            ENC_SetDistance(100, 100);
            ...
            else
            {
             ...
             ucMotorState = 0;//if working try 5 to see if it hard breaks
              ...
              if(CR1_ulMotorTimerNow - breakTimer >=500)
              {
                ENC_vi32RightOdometer = 200;//reset odometer
                ENC_vi32LeftOdometer = 200;//reset odometer
                CR1_ulMotorTimerPrevious=millis();
                state = 10;
              }
            }
          }
          else if(state == 10)//Move Reverse
          {
            if( ENC_vi32RightOdometer >0)//start correction
            {
            //set up a boolean of should be forward should be backwards
            //if the encoders should be forward
            ENC_SetDistance(100, 100);
            ucMotorState = 6;   //backwards
            CR1_ui8LeftWheelSpeed = CR1_ui8WheelSpeed;
            CR1_ui8RightWheelSpeed = CR1_ui8WheelSpeed;
            breakTimer=millis();
            }
          ...
          else if (state == 11)
            {
              ...
              else
              {
                ...
                move(1);
                CR1_ulMotorTimerNow = millis();//start a timer
                if (CR1_ulMotorTimerNow - breakTimer <=3000)
                servoPos=0;//Need to fix channel or else won't work
                else
                servoPos=90;
                ledcWrite(servoChannel, degreesToDutyCycle(servoPos));  // set the desired servo position
                //Add flag rai
      ...
      if(btRun)
      {
          if (CR1_ui8IRDatum == 0x55) {                // if proper character is seen, move forward
            ENC_SetDistance(511, 511);
            ucMotorState = 1;   //forward
            CR1_ui8LeftWheelSpeed = CR1_ui8WheelSpeed;
            CR1_ui8RightWheelSpeed = CR1_ui8WheelSpeed;
       }
      ...
      */
      ...
      {
       CR1_ulMotorTimerNow = millis();
       if(CR1_ulMotorTimerNow - CR1_ulMotorTimerPrevious >= CR1_ciMotorRunTime)//This makes our code timer based which is currently 1s, might want to expand or even take away time dependance   
       {   
         ...
         switch(ucMotorStateIndex)
         {
           ...
           case 1:
          {
           ...
           case 2:
          {
            ucMotorStateIndex = 7;//changed to go straight to reverse
            ucMotorState = 0;
            move(0);
            break;
          }
