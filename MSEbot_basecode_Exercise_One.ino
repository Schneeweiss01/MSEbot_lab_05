...
//   NEO_RGBW    Pixels are wired for RGBW bitstream (NeoPixel RGBW products)
   ...
   int ForB = 0; //new variable, should be 0 when trying to go forwards, 1 when trying to go backwards, 2 when ready to commence the finale
...
//*************************************************************************************************    
   switch(CR1_ucMainTimerCaseCore1)  //full switch run through is 1mS
   {
    ...
    case 0: 
    {
      ...
      if (btRun)
      {
        if (ForB == 0)//if bot should be moving forward
        {
           ...
           if( ENC_vi32RightOdometer <40)//start correction
            ...
            else if( ENC_vi32RightOdometer <=900)
            {
            //set up a boolean of should be forward should be backwards
            //if the encoders should be forward
            ENC_SetDistance(100, 100);
            ...
            else
            {
              ...
              move(1);
              ForB = 1;
              //WSVR_BreakPoint(1);//THIS IS THE BREAKPOINT WE MIGHT NEED TO USE
            }
          }
          else if (ForB ==1)//when bot is ready to go backwards
          {
            if(ENC_vi32RightOdometer >870)
            {
            ENC_SetDistance(-100, -100);
            ...
            else if( ENC_vi32RightOdometer >500)
            {
            //set up a boolean of should be forward should be backwards
            //if the encoders should be forward
            ENC_SetDistance(-100, -100);
            ...
            else
            {
              ...
              CR1_ulMotorTimerPrevious = millis();//start a timer
            }
            }
            else if (ForB == 2)
            {
              ...
              else
              {
                ...
                move(1);
                CR1_ulMotorTimerNow = millis();//start a timer
                if (CR1_ulMotorTimerNow - timer <=3000)
   ...
   //   asm volatile("esync; rsr %0,ccount":"=a" (vui32test2)); // @ 240mHz clock each tick is ~4nS 
    ...
    {
       if (CR1_ui8IRDatum == 0x55 && ForB==0) {                // if proper character is seen
         SmartLEDs.setPixelColor(0,0,25,0);         // make LED1 green with 10% intensity
       }
       else if (CR1_ui8IRDatum == 0x41 || ForB==1) {           // if "hit" character is seen
